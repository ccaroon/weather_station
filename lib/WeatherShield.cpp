...
float WeatherShield::readAltitude() {
  toggleOneShot(); // Toggle the OST bit causing the sensor to immediately take
  // another reading
...
float WeatherShield::readAltitudeFt() { return (readAltitude() * 3.28084); }
...
float WeatherShield::readPressure() {
  ...
  pressure_whole >>= 6; // Pressure is an 18 bit number with 2 bits of decimal.
...
void WeatherShield::enableEventFlags() {
  IICWrite(PT_DATA_CFG, 0x07); // Enable all three pressure and temp event flags
}
...
void WeatherShield::windSpeedIRQ() {
  // Ignore switch-bounce glitches less than 10ms (142MPH max reading) after the
  // reed switch closes

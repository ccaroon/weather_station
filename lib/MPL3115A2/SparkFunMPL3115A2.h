...
class MPL3115A2 {
  ...
  public:
    ...
    float readPressure(); // Returns float with barometric pressure in Pa. Ex:
                          // 83351.25
    float
    readTemp(); // Returns float with current temperature in Celsius. Ex: 23.37
    float readTempF(); // Returns float with current temperature in Fahrenheit.
                       // Ex: 73.96
    ...
    void setModeStandby();   // Puts the sensor into Standby mode. Required when
                             // changing CTRL1 register.
    void setModeActive();    // Start taking measurements!
    void setOversampleRate(
        byte); // Sets the # of samples from 1 to 128. See datasheet.
    void enableEventFlags(); // Sets the fundamental event flags. Required
                             // during setup.
